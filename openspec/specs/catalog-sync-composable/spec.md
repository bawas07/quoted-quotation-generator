...
1. For each item with `change_type: "NEW"`:
   - Calls `useCatalog().addItem()` with:
     ...
     - `cur`: from the current quotation's currency (passed via a parameter)
     - `unit`: empty string
2. For each item with `change_type: "UPDATE"` and `match` present:
   ...
   - Re-reads the live catalog entry to avoid stale data when multiple items match the same entry
   - Calls `useCatalog().updateItem(entryId, { price, history, times })` with the updated history and incremented times
