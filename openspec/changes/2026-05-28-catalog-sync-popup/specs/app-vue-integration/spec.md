...
The current `@update:modelValue` handler on `StatusSelector` (`(val) => setStatus(val)`) MUST be replaced with a handler that:
...
- Set `isDirty = true` (the `useQuotation`'s `setStatus` already calls `markDirty()`)
