...
`CatalogSyncPopup` MUST use `AppModal` as its shell. The popup controls modal open/close via a `modelValue` / `update:modelValue` v-model pattern, with props `open` and emit `close`.
...
- **AND** shows `was Rp 1,800,000 · Mar 20, 2026 · Beta Studio` in `.sync-hist`
...
- **THEN** the item's checked state updates locally (emitted or mutated via v-model)
...
- **WHEN** user unchecks all
- **THEN** button shows "Save Selected (0)"
...
Buttons styled per `docs/example-v2.html` `.modal-ftr .btn-*` patterns.
...
All price displays (`.sync-price`, `.sync-hist`) MUST use the `formatCurrency` utility from `src/utils/formatCurrency.ts` for consistent currency formatting.

#### Scenario: Price formatted
...
- **THEN** the HTML structure matches `docs/example-v2.html` lines 1146-1158
